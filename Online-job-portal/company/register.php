...
<body>
  ...
  <div class="container">
    <form method="POST" enctype="multipart/form-data">
      ...
      <label for="fname">Registration ID</label> <br>
      <input type="text" name="id"> <br>

      <label for="fname">CEO Name</label> <br>
      <input type="text" name="ceoname"> <br>
  ...
  <?php
    if(isset($_POST['cname'])==true)
    {
        ...
        $logoname=$_FILES['file']['tmp_name'];
        $logo=$_FILES['file']['name'];

        if(move_uploaded_file($logoname,$logo))  
        {
        $con=mysqli_connect("localhost","root","root","job","3306");
        $q="insert into company values('$cname','$id','$ceoname','$location','$email','$phone','$password','../image/$logo')";
